...
from ..base_provider import AbstractProvider  # Base class for provider abstraction
...
class Equing(AbstractProvider):
    ...
    def create_completion(
        ...
        # Headers and JSON data for the API request
        headers = {
        ...
        if not stream:
            # Return the completion if not streaming
            yield response.json()["choices"][0]["message"]["content"]
            return

        # Process the response and yield the content if available
